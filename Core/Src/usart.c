...
/* USER CODE BEGIN 1 */
/**
  * @brief Init the interruption of all uart
  * @param void
  * @retval void
  */
...
/**
  ...
  * @param uartHandle the pointer of uart handle
  * @param format the string format
  * @param other the elements in format
  * @retval void
  */
void UART_TransmitString(UART_HandleTypeDef *uartHandle, char *format, ...) {
    ...
    va_list ap;
    va_start(ap, format);
    vsprintf(uart_send_buffer, format, ap);
    va_end(ap);
    len = strlen((const char *) (uart_send_buffer));
    UART_TransmitData(uartHandle, (uint8_t *) uart_send_buffer, len);
}
...
/**
  ...
  * @param other the length of data
  * @retval void
  */
