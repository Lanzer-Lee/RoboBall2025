/** @brief The SDK and User API of WIT IMU.
  * @author Lanzer
  * @date 2024-12-07
  * @version 1.0
  */
...
WIT_TypeDef wit_imu = {0};

volatile uint8_t s_cDataUpdate = 0, s_cCmd = 0xFF;

/** @brief Init WIT IMU
  * @param imu the pointer of IMU structure
  * @retval void
  */
void WIT_Init(WIT_TypeDef *imu) {
    imu->uart_handle = &huart2;
    imu->uart_buffer = &uart2_buffer;
    WitInit(WIT_PROTOCOL_NORMAL, 0x50);
    WitSerialWriteRegister(WIT_UartSendData);
    WitRegisterCallBack(WIT_UpdateData);
...
/** @brief Read data from IMU
  ...
  * @retval void
  */
void WIT_ReadData(WIT_TypeDef *imu) {
    int i = 0;
    if (s_cDataUpdate) {
        for (i = 0; i < 3; i++) {
            imu->acc[i] = (float) sReg[AX + i] / 32678.0f * 16.0f;
            imu->gyro[i] = (float) sReg[GX + i] / 32678.0f * 2000.0f;
            imu->angle[i] = (float) sReg[Roll + i] / 32678.0f * 180.0f;
        }
...
/** @brief Send data to IMU
  * @param data the address of data
  * @param size the length of data
  * @retval void
  */
void WIT_UartSendData(uint8_t *data, uint32_t size) {
    UART_TransmitString(wit_imu.uart_handle, data);
}

/** @brief Delay function of IMU
  * @param ms time
  * @retval void
  */
void WIT_Delay(uint16_t ms) {
    HAL_Delay(ms);
}
...
/** @brief Update IMU data
  ...
  * @retval void
  */
void WIT_UpdateData(uint32_t reg, uint32_t num_reg) {
    int i = 0;
    for (i = 0; i < num_reg; i++) {
        switch (reg) {
            case AZ:
...
/** @brief Detect IMU and link to it
  * @param void
  * @retval void
  */
void WIT_AutoScanSensor(void) {
    int i = 0, retry = 0;
    for (i = 1; i < 10; i++) {
        ...
        MX_USART2_UART_Init();
        HAL_UART_Receive_IT(wit_imu.uart_handle, &wit_imu.uart_buffer->byte, 1);
        retry = 2;
        do {
            s_cDataUpdate = 0;
            WitReadReg(AX, 3);
            HAL_Delay(100);
            if (s_cDataUpdate != 0) {
                USB_TransmitString("Find sensor\r\n\r\n");
                return;
            }
            retry--;
        } while (retry);
    }
    USB_TransmitString("can not find sensor\r\n");
}
            ...
            if (s_uiWitDataCnt > 2) {
                ...
                for (i = 0; i < usTemp; i++) {
                    sReg[i + s_uiReadRegIndex] =
                            ((uint16_t) s_ucWitDataBuff[(i << 1) + 3] << 8) | s_ucWitDataBuff[(i << 1) + 4];
                }
